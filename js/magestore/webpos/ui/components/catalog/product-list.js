    ...
    return Component.extend({
        ...
        _prepareItems: function () {
            ...
            deferred.done(function (data) {
                var configurableProducts = _.where(data.items, {'type_id': 'configurable'});
                var excludeProductIds = [];
                _.each(configurableProducts, function (configurableProduct, index) {
                    excludeProductIds =_.union(excludeProductIds,configurableProduct['child_ids']);
                });
                var preItems = data.items;
                data.items = _.filter(preItems, function (item) {
                    return !_.contains(excludeProductIds, (item.entity_id * 1));
                });
