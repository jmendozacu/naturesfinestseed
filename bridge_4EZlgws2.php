...
/*-----------------------------------------------------------------------------+
| eMagicOne																	|
| Copyright (c) 2012-2016 eMagicOne.com <contact@emagicone.com>				|
| All rights reserved														  |
+------------------------------------------------------------------------------+
|																			  |
| PHP MySQL Bridge															 |
...
$exclude_db_tables = 'log_*;dataflow_*;xcart_sessions_data;xcart_session_history;xcart_stats_shop;xcart_stats_pages_views;xcart_stats_pages;xcart_stats_pages_paths;amazonimport_*;bcse_catalog_sessions;bcse_catalog_config;google_*;zen_uti;zen_uti_*;emo_admin;emo_admin_*;emo_user_*;admin_activity_log;*connections*;*guest;*pagenotfound;ui_bookmark';
...
define('GET_SQL_FILE_PART_NAME', 'get_sql_file_part_name');
...
//$file_name_get_sql_tmp = 'sm_tmp_get_sql.txt';
...
define('DELAY_TO_GENERATE_DUMP', 10); // seconds
...
function get_sql_dump() {
    ...
    // Set data to initial at the start
    if (!cMySQLBackUp::getDumpData($temporary_dir)) {
...
function get_config() {
    ...
    $sConf = array(
        'database_host' => ((DB_SERVER == 'localhost' || strpos(DB_SERVER, 'localhost:') == 0) ? $GLOBALS['g_sCartHost'] : DB_SERVER ),
...
function get_file_part($fname, $position) {
	...
	if(!headers_sent()) {
		...
		header('Content-Length-Alternative: ' . (strlen($outpustr) + $fsize));
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Pragma: public');
		header('Pragma: no-cache');
...
function get_sql_file_part_info() {
    ...
    for ($i = 0; $i < 10; $i++) {
        sleep(10);
        $file_part = _get_generating_dump_value(GET_SQL_FILE_PART_NAME);
        ...
        if ($file_part && !empty($file_part)) {
            ...
            $dump = new cMySQLBackUp($temporary_dir, $file_part, $is_compressed, $package_size, true);
            $dump->download($file_part);
            _set_generating_dump_value(array(GET_SQL_FILE_PART_NAME => ''));
            die();
        }
    }
...
function get_store_file_archive() {
	...
	if (version_compare(phpversion(), '5.2.1', '<')) {
		die(json_encode(
			array(
				...
				'message' => 'It is supported in php version >= 5.2.0'
			)
		));
	}

	if (!is_writable($temporary_dir . '/')) {
		die(json_encode(
			array(
				'response_code' => ERROR_GENERATE_STORE_FILE_ARCHIVE,
				'message' => '"' . $temporary_dir . '" is not writable'
			)
		));
	}
	...
	if (!extension_loaded('zip')) {
		die(json_encode(
			array(
				'response_code' => ERROR_GENERATE_STORE_FILE_ARCHIVE,
				'message' => 'Please install zip extension'
			)
		));
	}
	...
	$source = !empty($_REQUEST['source_dir']) ? $_REQUEST['source_dir'] : './';
	...
	if ($zip->open($file, ZipArchive::CREATE) === true) {
		...
		generate_file_archive($zip, $source, $arr_ignore_dir);
		$zip->close();
...
function generate_file_archive($zip, $source, $arr_ignore_dir) {
	...
	if (is_dir($source)) {
		...
		foreach ($files as $file) {
			$file = realpath($file);
			if (!in_array($file, $arr_ignore_dir)) {
				if (is_dir($file)) {
...
function _set_generating_dump_value($arr) {
...
*******************************************************************/
	...
	*/
	function create_dump() {
        ...
        if (!$dump_data) {
            $res = $this->rLink->query('SELECT DEFAULT_CHARACTER_SET_NAME AS charset FROM information_schema.SCHEMATA WHERE SCHEMA_NAME = "'.DB_DATABASE.'"');
            if ($res) {
...
//                $this->fn_write($fp, "SET NAMES 'utf8';\n\n");
    ...
    private function generateArchive() {
        ...
        if ($allow_compression) {
//            $fname_gz = 'em1_bridge_db_dump.gz';
	...
	* @access  public
	*/
	function download($fname = '', $is_log = true) {
		if ($fname != '') {
		...
		if ($this->iPackageSize > 0) {
			$this->_file_parts($file_size, $this->sBackUpFile, $file_checksum);
			return;
		}
...
//		if(!$this->resetLog && !$dump_data) {
	...
	function _file_parts($file_size, $file_name, $file_checksum) {
		$outpustr = "0\r\n";
...
//		$include_db_tables = (isset($GLOBALS['include_db_tables']) ? (string)$GLOBALS['include_db_tables'] : '');
		$quoted_tbls = array();
		if(isset($_REQUEST['include_tables']) && !empty($_REQUEST['include_tables'])) {
...
function get_cart_version() {
	...
	if ($g_iCartType == 3) {
        ...
        require_once 'vendor/magento/framework/AppInterface.php';
        return '1|' . Magento\Framework\AppInterface::VERSION;
    }
