...
$exclude_db_tables = 'log_*;dataflow_*;xcart_sessions_data;xcart_session_history;xcart_stats_shop;xcart_stats_pages_views;xcart_stats_pages;xcart_stats_pages_paths;amazonimport_*;bcse_catalog_sessions;bcse_catalog_config;google_*;zen_uti;zen_uti_*;emo_admin;emo_admin_*;emo_user_*;admin_activity_log;*connections*;*guest;*pagenotfound';
...
function get_sql_dump() {
    ...
    if (cMySQLBackUp::getDumpData($temporary_dir) === false)
...
function getCartType() {
    ...
    // Woocommerce
    if( file_exists(dirname(__FILE__) . '/wp-content/plugins/woocommerce/woocommerce.php') ) {
        return 9;
    }
	return -1;
}
...
{
    global $temporary_dir, $file_name_get_sql_tmp;
...
class cMySQLBackUp {
	...
	var $PingCount = 0;
	var $sBackUpFile = 'em1_bridge_db_dump.sql';  // file name to make back up
...
*******************************************************************/
	...
	*/
	function create_dump() {
		...
		$result = $this->rLink->query("SET SQL_QUOTE_SHOW_CREATE = 1");
...
//					$total = $this->rLink->num_rows($result);
					...
					while(($result = $this->rLink->query($query)) && ($total = $this->rLink->num_rows($result))){
                        ...
                        if (_get_generating_dump_value(GET_SQL_CANCEL_PARAM))
                        {
                            ...
                            $path_sm_tmp_get_sql_txt = $this->sBackUpDir . '/' . 'sm_tmp_get_sql.txt';
                            $path_dump_data_tmp_txt = $this->sBackUpDir . '/' . 'dump_data_tmp.txt';
                            $path_em1_bridge_db_dump_sql = $this->sBackUpDir . '/' . 'em1_bridge_db_dump.sql';
...
//		if(!$this->resetLog && !$dump_data) {
	...
	*/
    ...
    private function putDumpData($table, $from)
    {
        ...
        file_put_contents($this->sBackUpDir . '/dump_data_tmp.txt', serialize($data));
    }

    public static function getDumpData($tmp_dir, $db_file = 'em1_bridge_db_dump.sql')
    {
        $content = false;
//        $file = $this->sBackUpDir . '/dump_data_tmp.txt';
        $file = $tmp_dir . '/dump_data_tmp.txt';
        $file_db = $tmp_dir . '/' . $db_file;
