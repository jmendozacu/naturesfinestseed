...
function get_store_file_archive() {
	...
	$arr_ignore_dir = isset($_REQUEST['ignore_dir']) && !empty($_REQUEST['ignore_dir'])
			? explode(';' ,$_REQUEST['ignore_dir'])
			: array();
	...
	if ($zip->open($file, ZipArchive::CREATE) === true) {
		$store_root_dir = './';
		generate_file_archive($zip, $store_root_dir, strlen($store_root_dir), $arr_ignore_dir);
		$zip->close();
...
function generate_file_archive($archive, $path, $store_root_dir_length, $arr_ignore_dir) {
	$skip = array('.', '..');
	...
	if ($fp = opendir($path)) {
		while ($value = readdir($fp)) {
			...
			if (is_file($item)) {
				...
				generate_file_archive($archive, $item, $store_root_dir_length, $arr_ignore_dir);
			}
		}
