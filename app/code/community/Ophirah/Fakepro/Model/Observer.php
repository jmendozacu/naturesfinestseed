 ...
 */
class Ophirah_Fakepro_Model_Observer
{
     ...
     */
    public function licensecheckFakeProduct(Varien_Event_Observer $observer)
    {
        ...
        if ($request->has('item') && !$request->getPost('update_items') && !($request->getActionName() == 'save')) {
            ...
            foreach ($items as $itemId => $itemOptions) {
                ...
                if ($product) {
                    if ($product->getSku() == Ophirah_Fakepro_Helper_Data::PRODUCT_SKU) {

                        if (!Mage::helper('qquoteadv/licensechecks')->isAllowedFakeProduct()) {
                            $message = Mage::helper('qquoteadv')->__("Custom product is only available in the Cart2Quote enterprise edition. <a href='https://www.cart2quote.com/cart2quote-update-upgrade.html'>Upgrade</a>");
     ...
     * @param Varien_Event_Observer $observer
     */
    public function createFakeProductObserver(Varien_Event_Observer $observer)
    {
        if (!Mage::helper('qquoteadv/licensechecks')->isAllowedFakeProduct()) {
            return;
        }
        ...
        if (Mage::getStoreConfig('custom_products/general/convert') == 2) {
            $request = $observer->getRequestModel();
            $this->createFakeProduct($request);
        }
    }

    /**
     * Function that check if the fake products on a quote need to be converted to a real product
     * This function only needs a quote object to be available in the $observer object
     *
     * @param Varien_Event_Observer $observer
     */
    public function convertFakeProductOnCreationObserver(Varien_Event_Observer $observer)
    {
        if (!Mage::helper('qquoteadv/licensechecks')->isAllowedFakeProduct()) {
            return;
        }
     ...
     * @param Varien_Event_Observer $observer
     */
    public function convertFakeProductObserver(Varien_Event_Observer $observer)
    {
        if (!Mage::helper('qquoteadv/licensechecks')->isAllowedFakeProduct()) {
            return;
        }
     ...
     * @param Varien_Event_Observer $observer
     */
    public function convertFakeProductFrontendObserver(Varien_Event_Observer $observer)
    {
        if (!Mage::helper('qquoteadv/licensechecks')->isAllowedFakeProduct()) {
            return;
        }
    ...
    /**
     * Replace the options on the fake product with hard values
     *
     * @param $qqadvproduct
     * @param $newProduct
     * @return array
     */
    private function _convertFakeProduct($qqadvproduct, $newProduct)
    {
        ...
        //save
        $newProduct->setShortDescription(null); //unset default fake product short description
        $newProduct->save();
    }
    ...
    private function _getConvertedSku($someId = null)
    {
        ...
        while ($productWithSameSKUExists) {
            $convertedSKU = Ophirah_Fakepro_Model_Observer::CONVERT_PRODUCT_SKU . '-' . $someId . rand(100, 999);
    ...
    private function _replaceValuesWithOptions($newProduct, $productOptions)
    {
        ...
        foreach ($newProductOptions as $newProductOptionId => $newProductOption) {
            //name option
            if ($newProductOption->getTitle() == Ophirah_Fakepro_Helper_Data::NAME) {
            ...
            //sku option
            if ($newProductOption->getTitle() == Ophirah_Fakepro_Helper_Data::SKU) {
                if (isset($productOptions[$newProductOptionId])) {
            ...
            //description option
            if ($newProductOption->getTitle() == Ophirah_Fakepro_Helper_Data::DESCRIPTION) {
                if (isset($productOptions[$newProductOptionId])) {
            ...
            //image option
            if ($newProductOption->getTitle() == Ophirah_Fakepro_Helper_Data::IMAGE) {
                if (isset($productOptions[$newProductOptionId]) && isset($productOptions[$newProductOptionId]['fullpath'])) {
    ...
    private function _replaceImageUploadOptions($newProduct, $requestOptions, $oldProduct)
    {
        ...
        foreach ($oldProductOptions as $oldProductOptionId => $oldProductOption) {
            if ($oldProductOption->getTitle() == Ophirah_Fakepro_Helper_Data::IMAGE) {
    ...
    {
        //remove the default fake product options and set it as data values
     ...
     * @throws Exception
     */
    public function createFakeProduct($request)
    {
        if (!Mage::helper('qquoteadv/licensechecks')->isAllowedFakeProduct()) {
            return;
        }
     ...
     * @param $quote
     */
    public function convertFakeProduct($quote)
    {
        //get quote products
        if (Mage::helper('qquoteadv/licensechecks')->isAllowedFakeProduct()) {
            ...
            foreach ($qqadvproductData as $qqadvproduct) {
                ...
                foreach ($quoteItems as $quoteItem) {
                    ...
                    if ($product->getSku() == Ophirah_Fakepro_Helper_Data::PRODUCT_SKU) {
                        ...
                        //move product options to product data for fake product
                        $this->_convertFakeProduct($qqadvproduct, $newProduct);
