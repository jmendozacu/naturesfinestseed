...
final class Ophirah_Qquoteadv_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getQuoteItem(
        ...
        $attributes,
        $quote = null,
        Ophirah_Qquoteadv_Model_Qqadvproduct $quoteProduct = null,
        $originalPrice = false,
        $skipOptions = true
    )
    {
        ...
        if($productParams['options']){
            foreach ($productParams['options'] as $key => $value){
                ...
                if(isset($value['size']) && isset($value['fullpath'])){
                    ...
                    $processingParams->setCurrentConfig($productParams);
        ...
        if ($quote == null) {
            ...
            //set the cutomer group id of the quote on the virtual quote
    ...
    public function updatePriceOnAddress($product, $addresses)
    {
        ...
        $priceIncludeTaxInCatalog = Mage::helper('tax')->priceIncludesTax($storeId); //Mage::getStoreConfig('tax/calculation/price_includes_tax', $storeId);
 ...
 */
    ...
    public function executeArticleRequest()
    {
        try {
            ...
            $response = $client->restGet('/api/v2/help_center/sections/' . $section . '/articles.json', $getValue); // API Request
