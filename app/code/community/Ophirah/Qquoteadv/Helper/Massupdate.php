...
class Ophirah_Qquoteadv_Helper_Massupdate extends Mage_Core_Helper_Data {
     ...
     * @throws Exception
     */
    public function update($type, array $products, $newSettings, $websiteId){
        if(in_array($type, $this->getMassUpdateTypes())){
            ...
            if($this->hasTable() && $newSettings) {
                $this
                    ...
                    ->updateProducts($products, $newSettings, $websiteId);
            }
        }
    }
     ...
     * @return $this
     */
    protected function updateProducts(array $products, $newSettings, $websiteId)
    {
        $newSettingsCollection = $this->prepareNewSettings($newSettings, $websiteId);
        while ($this->incrementingProducts() && count($newSettingsCollection)) {
            $chunkOfProducts = $this->getArrayPiece($products);

            $this->deleteCurrentSettings($websiteId, $chunkOfProducts);
     ...
     * @return string
     */
    protected function getCleanUpSql($products, $websiteId){
        $removeProductData = "";
        $deleteSql = "DELETE FROM `".$this->_currentTable."` WHERE ( `website_id` = '".$websiteId."' AND `entity_id` = '";
     ...
     * @return array
     */
    protected function prepareNewSettings(array $tableData, $websiteId){
        $newTableData = array();
     ...
     * @return array
     */
    public static function getMassUpdateTypes(){
        return array(
     ...
     * @return array
     */
    public static function getMassPostTypes($updateType){
        $postFields =  array(
     ...
     */
    protected function deleteCurrentSettings($websiteId, $chunkOfProducts)
    {
        $this->getConnection()->query(
            $this->getCleanUpSql($chunkOfProducts, $websiteId)
        );
    }
