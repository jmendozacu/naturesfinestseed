...
//        $this->setTemplate('qquoteadv/product.phtml'); @deprecated: now managed in the qquoteadv.xml layout file
     ...
     * @return array
     */
    public function getQtyHtml($collection, $disabledHtmlRadio, $inputDisabled,  $product, $q2qKey, $disabledHtml, $_qTotals){
        $hideRadio = true;
        $prices = array();
        $margins = array();
        ...
        if (count($collection) > 0):
            ...
            foreach ($collection as $requested_item):
                ...
                $ownerPrice = Mage::app()->getStore()->roundPrice($requested_item->getOwnerCurPrice());
