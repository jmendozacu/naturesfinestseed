...
class Ophirah_Qquoteadv_Model_Email_Autoproposal extends Mage_Core_Model_Abstract
{
     ...
     * @return bool
     */
    public function isAllowed($includeLimits = true)
    {
    ...
    protected function prepareEmail()
    {
        ...
        $this->_template
            ...
            ->setTemplateSubject($this->_template['template_subject'])
            ->setData('c2qParams', array('email' => $this->getQuote()->getEmail(), 'name' => $this->getCustomer()->getName()));
    ...
    protected function addBcc()
    {
        ...
        if ((bool)Mage::getStoreConfig('qquoteadv_quote_emails/sales_representatives/send_linked_sale_bcc', $this->getStoreId())) {
            $this->_template->addBcc(Mage::getModel('admin/user')->load($this->getQuote()->getUserId())->getEmail());
        }
        return $this;
    }
     ...
     * @return Ophirah_Qquoteadv_Model_Qqadvcustomer
     */
    public function setQuoteStatus($quote = null)
    {
        if (!isset($quote)) {
            $quote = $this->getQuote();
        }
        if ($this->isAllowed() && $quote instanceof Ophirah_Qquoteadv_Model_Qqadvcustomer) {
