...
class Mirasvit_Advr_Model_Report_Sales extends Mirasvit_Advr_Model_Report_Abstract
{
    ...
    public function joinView()
    {
        ...
        if (!isset($this->joinedTables[$tableName])) {
            $this->getSelect()->joinLeft(
                array(
                    $tableName => new Zend_Db_Expr(
                        '(SELECT product_id, COUNT(visitor_id) as qtyVisited, COUNT(customer_id) as qtyCustomerVisited'.
                        ' FROM '.$this->getTable('reports/viewed_product_index').
                        ' GROUP BY product_id)'
                    ),
                ),
                $tableName.'.product_id = catalog_product_table.entity_id',
                array('qtyVisited', 'qtyCustomerVisited')
            );
    ...
    public function setFilterData($data, $filterByStatus = true, $timeOfGroup = true)
    {
